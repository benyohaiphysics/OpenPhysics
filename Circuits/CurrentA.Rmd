...
Calculate the current in each of the following circuits;
...
for (i in 1:numberOfImages) {
  resistance<-sample(0:100,1)*10^(sample(-3:6,1))
  voltage<-sample(0:100,1)*10^(sample(-9:6,1))
...
component<-c("to[battery]",paste("to[R=\\SI{",formatEng(resistance),"}{\\ohm}]"),paste("to[short] ",if(position[3]==1){
  ...
  ,' node[component]{A} to[short]'),'to[short]')
...
plotstring<-paste(i,")\\newline",sprintf(template,factor(circuit[which(position=='1'),]$component),factor(circuit[which(position=='2'),]$component),factor(circuit[which(position=='3'),]$component),factor(circuit[which(position=='4'),]$component),
          ...
          "(0,0.5)--(-2,0.5) 
          to[short] (-2,2) node[component,label=left:\\SI{",formatEng(voltage),"}{\\volt}]{V} to[short] 
          (-2,3.5)--(0,3.5)")
  }else if(position[2]==2){paste("(0.5,4)--(0.5,6) 
          to[short] (2,6) node[component,label=\\SI{",formatEng(voltage),"}{\\volt}]{V} to[short] 
          (3.5,6)--(3.5,4)")
  } else if(position[2]==3){paste("(4,3.5)--(6,3.5) 
          to[short] (6,2) node[component,label=right:\\SI{",formatEng(voltage),"}{\\volt}]{V} to[short] 
          (6,0.5)--(4,0.5)")
    }else if(position[2]==4){paste("(3.5,0)--(3.5,-2) 
          to[short] (2,-2) node[component,label=below:\\SI{",formatEng(voltage),"}{\\volt}]{V} to[short] 
          (0.5,-2)--(0.5,0)")
  }       )       )
...
answers<-c(answers,voltage/resistance)
}
