...
Calculate the potential difference across the primary, $V_p$ or secondary coil $V_s$. The number of turns *drawn* on the diagram aren't accurate;
...
\\node[right,text width=1.9cm] at (0,-0.4) {$N_p$ = %s turns};
\\node[right,text width=1.9cm] at (4,-0.4) {$N_s$ = %s turns};
\\end{scope}
...
plotstring<-paste(i,")\\newline",sprintf(template,primarySpacing[2],primarySpacing[3],if(hidePrimary==TRUE)("")else(paste(" = \\SI{",formatEng(signif(primaryVoltage,2)),"}{\\volt}")),secondarySpacing[2],secondarySpacing[3],if(hidePrimary==FALSE)("")else(paste(" = \\SI{",formatEng(signif(secondaryVoltage,2)),"}{\\volt}")),primaryTurns,secondaryTurns,sep=""))
...
answers<-c(answers,if(hidePrimary==TRUE)(paste("$V_p$ = \\SI{",formatEng(signif(primaryVoltage,2)),"}{\\volt}"))else(paste("$V_s$ = \\SI{",formatEng(signif(secondaryVoltage,2)),"}{\\volt}")))
