...
Calculate number of turns on  the primary, $N_p$ or secondary coil $N_s$. The number of turns *drawn* on the diagram aren't accurate and assume the transformer is 100% efficient;
...
\\draw[latex-latex] (-\\lx, \\ly) --++(0, 2.8);
%% node[midway, left, text width=1.1cm] {$V_p$}
...
\\draw[latex-latex] (2*\\a+\\lx, \\ly) --++(0, 3.1);
%% node[midway, left, text width=1.1cm] {$V_s$};
...
plotstring<-paste(i,")\\newline",sprintf(template,primarySpacing[2],primarySpacing[3],paste(" = \\SI{",formatEng(signif(primaryCurrent,3)),"}{\\ampere}"),paste(" = \\SI{",formatEng(signif(secondaryCurrent,3)),"}{\\ampere}"),secondarySpacing[2],secondarySpacing[3],if(hidePrimary==TRUE)("")else(paste("= ",primaryTurns," turns")),if(hidePrimary==FALSE)("")else(paste("= ",secondaryTurns," turns")),sep=""))
